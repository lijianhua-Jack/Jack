...
function baocun(a) {
	...
	var name = $tr.children("td:eq(2)").children("input").val(); // 商品名
	var spec = $tr.children("td:eq(3)").children("input").val(); // 型号
	var price = $tr.children("td:eq(4)").children("input").val(); // 价格
	var amount = $tr.children("td:eq(5)").children("input").val(); //数量
	
	var parm = {"goodsid":goodsid, "name":name, "spec":spec,
...
function xiugai(a) {
	...
	// 对tr中中间的几个可以修改的单元格镜星文本替换
	var $td3 = $tr.children("td:eq(2)");
	var $td4 = $($tr.children("td")[3]);
	var $td5 = $($tr.children("td")[4]);
	var $td6 = $($tr.children("td")[5]);
	
	editTd($td3); // 调用函数修改单元格编程文本框
	editTd($td4); // 调用函数修改单元格编程文本框
	editTd($td5); // 调用函数修改单元格编程文本框
	editTd($td6); // 调用函数修改单元格编程文本框
...
function deldata(t){
	...
	var goodsid = $t.parent().parent().parent().children("td:eq(0)").children("input").val(); // 商品id
	// console.log(goodsid);
	var parm = {"goodsid":goodsid};
